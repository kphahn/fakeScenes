# Code forked from the EUFS_sim
# Al rights goes to the original authors of the code
# https://gitlab.com/eufs/eufs_sim/-/blob/68079397177909e1ea8b2ace68b16b678095fc4f/eufs_tracks/eufs_tracks/track_generator/track_generator.py
...
class TrackGenerator:
    ...
    def generate_path_w_params(
        ...
        waves = np.zeros(n_points, dtype=np.csingle)
        dwaves = np.zeros(n_points, dtype=np.csingle)
...
def generate_cfg_from_seed(seed) -> dict:
    rand = random.Random(seed)
    cfg = {
        'seed': seed,
        'length': rand.randint(100, 300),
        'min_corner_radius': rand.uniform(3, 5),
        'max_frequency': 7,
        'amplitude': 1 / 3,
        'rel_accuracy': 0.005,
        'margin': 0,
        ...
        'starting_straight_downsample': 2,
        'min_cone_spacing': rand.uniform(1.0, 3.0),
        'max_cone_spacing': rand.uniform(4.0, 7.0),
        'track_width': rand.uniform(3.0, 5.0),
        'cone_spacing_bias': rand.uniform(0.3, 0.6),
        'starting_cone_spacing': rand.uniform(0.5, 0.7)
    }

    return cfg

def generate_track_data_from_cfg(cfg) -> dict:
    gen = TrackGenerator(cfg)
    path = gen.gen_path()
    path = gen.pick_starting_point(
        *path,
        starting_straight_length=cfg['starting_straight_length'],
        downsample=cfg['starting_straight_downsample']
    )

    start_cones, l_cones, r_cones = gen.gen_cones(path)
    ...
    for p in path[0]:
        data["path"].append([p.real, p.imag])
    for c in l_cones:
        data["blue_cones"].append([c.real, c.imag])
    for c in r_cones:
        data["yellow_cones"].append([c.real, c.imag])
    for c in start_cones:
        data["big_cones"].append([c.real, c.imag])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--output", type=str, default="generated/tracks", help="Output directory for generated .json tracks")
    parser.add_argument("-n", "--num_tracks", type=int, default=10, help="Number of tracks to generate")
    parser.add_argument("-s", "--seed", type=int, required=False, help="Generate single track with specified seed")
    ...
    if args.seed:
        cfg = generate_cfg_from_seed(args.seed)
        data = generate_track_data_from_cfg(cfg)
        with open(data_dir / f"track_s{args.seed}.json", "w") as f:
            json.dump(data, f, indent=4)
    else:
        for i in range(args.num_tracks):
            seed = random.randint(0, 2**32 - 1)
            print(f"Generating track {i} with seed {seed}")

            cfg = generate_cfg_from_seed(seed)
            data = generate_track_data_from_cfg(cfg)

            with open(data_dir / f"track_{i}.json", "w") as f:
                json.dump(data, f, indent=4)
